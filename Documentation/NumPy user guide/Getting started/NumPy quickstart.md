...
The second rule of broadcasting ensures that arrays with s size of 1 along a particular dimension act as if they had the size of the array with the largest shape along that dimension. The value of the array element is assumed to be the same along that dimension for the "broadcasting" array.
...
>>> a[i] # the element of 'a' at the positions 'i'
...
```python
>>> a[b] = 0 # All element of 'a' higher than 4 become 0
>>> a
array([[0, 1, 2, 3],
       [4, 0, 0, 0],
       [0, 0, 0, 0]])
...
You could be also implement the reduce as follows:
